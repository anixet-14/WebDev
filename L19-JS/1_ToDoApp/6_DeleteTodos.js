...
//1.Detect which cross button has been clicker
taskList.addEventListener('click', (ev)=>{
    let target = ev.target
    if(target.classList.contains('delete')){
        console.log(target.parentElement) //This returns complete li tag,i.e. parent tag of button tag which is clicked, so we will simply remove it to remove from html
        target.parentElement.remove()

        //Removing data from todos as well (Usually we only delete from todos and refresh it, but we doing two different task for learning purposes)
        let taskName= target.parentElement.innerText;
        console.log(taskName)
        
        //Finding class of element
